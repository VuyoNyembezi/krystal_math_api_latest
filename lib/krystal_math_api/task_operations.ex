defmodule KrystalMathApi.TaskOperations do
  ...
  defp open_team_task_search(team_id, search_term) do
    ...
    Repo.all(
      from(t in Task,
        where:
          like(t.name, ^search_name) and t.due_date >= ^DateTime.utc_now() and t.active == true and
            t.team_id == ^team_id
      )
    )
    |> Repo.preload([:team, :user, :task_status, :environment])
  ...
  defp active_user_task_search(team_id, user_id, search_term) do
    ...
    Repo.all(
      from(t in Task,
        where:
          like(t.name, ^search_name) and t.active == true and t.team_id == ^team_id and
            t.user_id == ^user_id
      )
    )
    |> Repo.preload([:team, :user, :task_status, :environment])
  ...
  defp not_active_user_task_search(team_id, user_id, search_term) do
    ...
    Repo.all(
      from(t in Task,
        where:
          like(t.name, ^search_name) and t.active == false and t.team_id == ^team_id and
            t.user_id == ^user_id
      )
    )
    |> Repo.preload([:team, :user, :task_status, :environment])
  ...
  defp open_user_task_search(team_id, user_id, search_term) do
    ...
    Repo.all(
      from(t in Task,
        where:
          like(t.name, ^search_name) and t.due_date >= ^DateTime.utc_now() and t.active == true and
            t.team_id == ^team_id and t.user_id == ^user_id
      )
    )
    |> Repo.preload([:team, :user, :task_status, :environment])
  end
  ...
  defp user_active_tasks(team_id, user_id) do
    query =
      from(t in Task,
        where:
          t.due_date >= ^DateTime.utc_now() and t.user_id == ^user_id and t.team_id == ^team_id and
            t.active == true
      )
  ...
  defp user_all_not_active_tasks(team_id, user_id) do
    query =
      from(t in Task, where: t.user_id == ^user_id and t.team_id == ^team_id and t.active == false)
  ...
  defp user_all_active_tasks(team_id, user_id) do
    query =
      from(t in Task, where: t.user_id == ^user_id and t.team_id == ^team_id and t.active == true)
  ...
  def user_tasks_counter(team_id, user_id) do
    query =
      from(t in Task, select: count(t.id), where: t.team_id == ^team_id and t.user_id == ^user_id)
  ...
  def user_task_completion(team_id, user_id) do
    user_tasks = %{
      completed: user_completed_tasks_counter(team_id, user_id),
  ...
  def user_tasks_statuses(team_id, user_id) do
    user_tasks_status = %{
      not_started: user_tasks_not_started(team_id, user_id),
  ...
  def team_task_completion(team_id) do
    team_tasks = %{
      completed: team_completed_tasks(team_id),
