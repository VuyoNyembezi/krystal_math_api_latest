...
######### Search Methods #########
  ...
  ###  Team AssignProject ####
    ...
    def get_team_project_assignments_project_type(conn, %{"team_id" => team_id,"id" => project_type}) do
  ...
  def assign_project(conn, %{"projects_assignment" => project_assignment_params}) do
    ...
    def update_assignment_details(conn, %{"id" => id, "projects_assignment" => project_assignment_params}) do
      projects_assignment = ProjectAssignments.get_assignment_project!(id)
      with {:ok, %ProjectAssignment{} = projects_assignment} <- ProjectAssignments.update_project_assign(projects_assignment, project_assignment_params) do
        conn
        |> put_status(200)
 ...
 #delete assignement Record/ removes assignment record from the system
  ...
  def un_assign_member(conn, %{"id" => id}) do
    projects_assignment = ProjectAssignments.get_assignment_project!(id)
    with {:ok,%ProjectAssignment{} = projects_assignment} <- ProjectAssignments.delete_assign_member(projects_assignment) do
      conn
      |> put_status(200)
