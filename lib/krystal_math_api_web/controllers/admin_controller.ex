defmodule KrystalMathApiWeb.AdminController do
  ...
  def create_environment(conn, %{"environment_attrs" => environment_params}) do
    with {:ok, %Environment{} = environment} <-
           AdminOperations.add_environment(environment_params) do
      conn
      |> put_status(:created)
  ...
  def update_environment(conn, %{"id" => id, "environment_attrs" => environment_params}) do
    ...
    with {:ok, %Environment{} = environment} <-
           AdminOperations.update_environment(environment, environment_params) do
  ...
  def update_task_status(conn, %{"id" => id, "task_status_attrs" => task_status_params}) do
    ...
    with {:ok, %TaskStatus{} = task_status} <-
           AdminOperations.update_task_status(task_status, task_status_params) do
  ...
  def update_user_status(conn, %{"id" => id, "user_status_attrs" => user_status_params}) do
    ...
    with {:ok, %UserStatus{} = user_status} <-
           AdminOperations.update_user_status(user_status, user_status_params) do
  ...
  def create_project_status(conn, %{"status" => project_status_params}) do
    with {:ok, %Status{} = project_status} <-
           AdminOperations.add_project_status(project_status_params) do
      conn
      |> put_status(:created)
  ...
  def update_project_status(conn, %{"id" => id, "status" => project_status_params}) do
    ...
    with {:ok, %Status{} = project_status} <-
           AdminOperations.update_project_status_value(project_status, project_status_params) do
  ...
  def create_project_type(conn, %{"project_type_attrs" => project_type_params}) do
    with {:ok, %ProjectType{} = project_type} <-
           AdminOperations.create_project_type(project_type_params) do
      conn
      |> put_status(:created)
  ...
  def update_project_type(conn, %{"id" => id, "project_type_attrs" => project_type_params}) do
    ...
    with {:ok, %ProjectType{} = project_type} <-
           AdminOperations.update_project_type(project_type, project_type_params) do
  ...
  def create_priority(conn, %{"priority" => priority_type_params}) do
    with {:ok, %Priority{} = priority_type} <-
           AdminOperations.create_priority(priority_type_params) do
      conn
      |> put_status(:created)
  ...
  def update_priority(conn, %{"id" => id, "priority" => priority_type_params}) do
    ...
    with {:ok, %Priority{} = priority_type} <-
           AdminOperations.update_priority(priority_type, priority_type_params) do
  ...
  def create_project_category_type(conn, %{"project_category" => project_category_params}) do
    with {:ok, %ProjectCategoryType{} = project_category_type} <-
           AdminOperations.create_project_category(project_category_params) do
      conn
      |> put_status(:created)
  ...
  def update_project_category_type(conn, %{
        "id" => id,
        "project_category" => project_category_params
      }) do
    project_category_type = AdminOperations.get_project_category_type!(id)

    with {:ok, %ProjectCategoryType{} = project_category_type} <-
           AdminOperations.update_project_category_type(
             project_category_type,
             project_category_params
           ) do
  ...
  def delete_project_category_type(conn, %{"id" => id}) do
    ...
    with {:ok, %ProjectCategoryType{}} <-
           AdminOperations.delete_project_category_type(project_category_type) do
      conn
      |> put_status(200)
